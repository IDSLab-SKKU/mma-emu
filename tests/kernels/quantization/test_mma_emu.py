...
The emulation reproduces a tensor core's fixed-function accumulation on CUDA
cores, so configured to match the GPU it runs on it must reproduce the native
CUTLASS result bit for bit. These tests check that through MmaEmu*Kernel vs
Cutlass*Kernel rather than the operators, which also covers weight preparation
and activation quantisation; operator-level checks (schema, rejected inputs)
...
def fp8_bias(n: int, out_dtype: torch.dtype, seed: int) -> torch.Tensor:
    """Seeded like the inputs are. A bias that nearly cancels the product is
    what exposes the last bits of the epilogue, so drawing it from the global
    generator would make which elements do that depend on how many tests ran
    first, and a failure would not reproduce on its own."""
    g = torch.Generator(device="cuda").manual_seed(seed)
    return torch.randn(n, generator=g, device="cuda", dtype=out_dtype)
...
def test_fp8_matches_native(m: int, n: int, k: int, out_dtype, use_bias: bool):
    ...
    bias = fp8_bias(n, out_dtype, n * 13 + m) if use_bias else None
    args = dict(
        A=a,
        B=b,
        out_dtype=out_dtype,
...
def test_fp8_m_sweep(use_bias: bool):
    ...
    for nk in range(32, 128, 32):
        bias = fp8_bias(nk, torch.bfloat16, nk) if use_bias else None
