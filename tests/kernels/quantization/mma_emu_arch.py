...
# paper. A failure on any other one is as likely to be the table's fault as the
# kernel's, and the header says so.
VERIFIED = {120}
...
NATIVE_REFERENCE = {
    ...
    "moe": ("cutlass_moe_mm", "mma_emu_moe_mm", 90),
}
...
def report_lines() -> list[str]:
    ...
    for fmt, (native_op, emu_op, min_sm) in NATIVE_REFERENCE.items():
        # MoE is built for exactly one architecture rather than a floor.
        supported = min_sm == SM if fmt == "moe" else min_sm <= SM
        if not _built(emu_op):
            lines.append(f"  {fmt:<6} not built for SM{SM} — skipped")
        elif not supported:
            lines.append(f"  {fmt:<6} needs SM{min_sm} — skipped")
        elif accumulation is None or fmt.replace("moe", "fp8") not in accumulation:
            lines.append(f"  {fmt:<6} no accumulation recorded for SM{SM} — skipped")
        else:
            config = accumulation[fmt.replace("moe", "fp8")]
            lines.append(
    ...
    if accumulation is None:
        ...
        lines.append(
            f"  SM{SM}'s accumulation comes from the paper and has not been "
            f"measured here, so a mismatch may be the table's fault"
        )
    return lines
