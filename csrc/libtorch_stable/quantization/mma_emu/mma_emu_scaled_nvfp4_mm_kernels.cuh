        ...
        for (int tm = 0; tm < THREAD_TILE_M; tm++) {
            ...
            for (int tn = 0; tn < THREAD_TILE_N; tn++) {
                ...
                for (int g = 0; g < GROUPS_PER_TILE; g++) {
                    ...
                    GroupResult gr =
                        fp4_gdfs_group_accumulate_predecoded<GS>(a_frag, b_frag,
                                                                 g_bits);
                    if (gr.all_zero) {
                        ...
                        tile_groups[g] = apply_ue4m3_scales(
                            gr.mantissa_sum, gr.max_exp, sfa, sfb, f_bits,
                            g_bits);
                    }
                }
 ...
 * @tparam OutDtype Output data type (__nv_bfloat16 or __half)
 */
template<int CS, typename OutDtype>
__launch_bounds__(NVFP4EmuConfig::NUM_THREADS)
        ...
        for (int tm = 0; tm < THREAD_TILE_M; tm++) {
            ...
            for (int tn = 0; tn < THREAD_TILE_N; tn++) {
                ...
                for (int chunk_start = 0; chunk_start < actual_k; chunk_start += CS) {
                    ...
                    accum[tm][tn] =
                        chunked_accumulate<CS>(chunk_operands, accum[tm][tn],
                                               f_bits);
                }
            }
        }
...
inline void dispatch_nvfp4(
    dim3 grid, dim3 block, cudaStream_t stream, const void* a, const void* b,
    void* out, const void* a_sf, const void* b_sf, const float* alpha,
  ...
  if (algorithm == design_space::kGDFS) {
    mma_emu_scaled_nvfp4_mm_emu_kernel<16, OutDtype><<<grid, block, 0, stream>>>(
        a_ptr, b_ptr, c_ptr, a_sf_ptr, b_sf_ptr, alpha, M, N, K, f_bits, g_bits);
  } else {
    mma_emu_scaled_nvfp4_mm_cofda_kernel<16, OutDtype><<<grid, block, 0, stream>>>(
        a_ptr, b_ptr, c_ptr, a_sf_ptr, b_sf_ptr, alpha, M, N, K, f_bits);
  }
}
...
inline void mma_emu_scaled_nvfp4_mm_emu(
    void* out, const void* a, const void* b, const void* a_sf, const void* b_sf,
    const float* alpha, int M, int N, int K, design_space::MmaEmuOutDtype out_dtype,
    int algorithm, int f_bits, int g_bits, cudaStream_t stream) {
