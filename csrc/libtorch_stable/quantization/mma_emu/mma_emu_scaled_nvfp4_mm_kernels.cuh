...
#pragma unroll
        for (int g = 0; g < GROUPS_PER_TILE; g++) {
          ...
          GroupResult gr =
              fp4_gdfs_group_accumulate_predecoded<GS>(a_frag, b_frag, g_bits);
          if (gr.all_zero) {
            ...
            tile_groups[g] = apply_ue4m3_scales(gr.mantissa_sum, gr.max_exp,
                                                sfa, sfb, f_bits, g_bits);
          }
        }
...
#pragma unroll
          ...
          accum[tm][tn] =
              chunked_accumulate<CS>(chunk_operands, accum[tm][tn], f_bits);
        }
      }
    }
...
inline void dispatch_nvfp4(dim3 grid, dim3 block, cudaStream_t stream,
  ...
  if (algorithm == design_space::kGDFS) {
    mma_emu_scaled_nvfp4_mm_emu_kernel<16, OutDtype>
        <<<grid, block, 0, stream>>>(a_ptr, b_ptr, c_ptr, a_sf_ptr, b_sf_ptr,
                                     alpha, M, N, K, f_bits, g_bits);
  } else {
    mma_emu_scaled_nvfp4_mm_cofda_kernel<16, OutDtype>
        <<<grid, block, 0, stream>>>(a_ptr, b_ptr, c_ptr, a_sf_ptr, b_sf_ptr,
                                     alpha, M, N, K, f_bits);
  }
}
