...
#pragma unroll
        for (int g = 0; g < GROUPS_PER_TILE; g++) {
          ...
          GroupResult gr =
              fp8_gdfs_group_accumulate_predecoded<GS>(a_frag, b_frag, g_bits);
          if (gr.all_zero) {
            tile_groups[g] = make_zero_operand();
          } else {
            tile_groups[g] = group_to_operand_fp8(gr.mantissa_sum, gr.max_exp,
                                                  f_bits, g_bits);
          }
        }
...
inline void launch_fp8_cofda(dim3 grid, dim3 block, cudaStream_t stream,
  ...
  switch (chunk_size) {
    case 16:
      mma_emu_scaled_fp8_mm_cofda_kernel<16, OutDtype>
          <<<grid, block, 0, stream>>>(a, b, c, scale_a, scale_b, bias, M, N, K,
                                       f_bits);
      break;
    case 32:
      mma_emu_scaled_fp8_mm_cofda_kernel<32, OutDtype>
          <<<grid, block, 0, stream>>>(a, b, c, scale_a, scale_b, bias, M, N, K,
                                       f_bits);
      break;
    default:
...
inline void launch_fp8_gdfs(dim3 grid, dim3 block, cudaStream_t stream,
  ...
  switch (group_size) {
    case 8:
      mma_emu_scaled_fp8_mm_gdfs_kernel<8, OutDtype>
          <<<grid, block, 0, stream>>>(a, b, c, scale_a, scale_b, bias, M, N, K,
                                       f_bits, g_bits);
      break;
    case 16:
      mma_emu_scaled_fp8_mm_gdfs_kernel<16, OutDtype>
          <<<grid, block, 0, stream>>>(a, b, c, scale_a, scale_b, bias, M, N, K,
                                       f_bits, g_bits);
      break;
    default:
