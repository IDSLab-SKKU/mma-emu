 ...
 * - C: Row-major [M x N]
 *
 * @tparam F Fractional bits F as a literal, or kRuntimeF to read it from the
 *           f_bits argument
...
__launch_bounds__(FP8EmuConfig::NUM_THREADS) __global__
    void mma_emu_scaled_fp8_mm_cofda_kernel(
        ...
        const OutDtype* __restrict__ bias, const int M, const int N,
        const int K, const int f_bits) {
  // The arithmetic below is reached only through __forceinline__ helpers, so a
  // literal here reaches them as one: the shift amounts and truncation masks F
  // drives stop being computed, and the sign tests guarding them collapse to
  // whichever branch survives. kRuntimeF passes f_bits through unchanged, which
  // is the code every unspecialized configuration still runs.
...
#pragma unroll
    for (int i = 0; i < A_ELEMS_PER_THREAD; i++) {
      ...
      if (idx < A_TILE_SIZE) {
        ...
        const __nv_fp8_storage_t raw = (global_row < M && global_col < K)
...
#pragma unroll
    for (int i = 0; i < B_ELEMS_PER_THREAD; i++) {
      ...
      if (idx < B_TILE_SIZE) {
        ...
        const __nv_fp8_storage_t raw = (global_row < K && global_col < N)
...
#pragma unroll
        for (int k_start = 0; k_start < BK; k_start += CHUNK_SIZE) {
          ...
          accum[tm][tn] =
              fp8_cofda_mma<CHUNK_SIZE>(a_frag, b_frag, accum[tm][tn], f);
        }
      }
    }
 ...
 *
 * @tparam F Inter-group fractional bits F as a literal, or kRuntimeF to read it
 *           from the f_bits argument
 * @tparam G GDFS intra-group bits G as a literal, or kRuntimeG to read it from
 *           the g_bits argument
...
__launch_bounds__(FP8EmuConfig::NUM_THREADS) __global__
    void mma_emu_scaled_fp8_mm_gdfs_kernel(
        const __nv_fp8_storage_t* __restrict__ A,  // [M, K] row-major
        const __nv_fp8_storage_t* __restrict__ B,  // [K, N] column-major
        OutDtype* __restrict__ C,                  // [M, N] row-major
        const float* __restrict__ scale_a_ptr,
        const float* __restrict__ scale_b_ptr,
        const OutDtype* __restrict__ bias, const int M, const int N,
        const int K, const int f_bits, const int g_bits) {
  // Literals here reach the arithmetic through __forceinline__ helpers, which
  // is what folds the shift amounts and truncation masks F and G drive.
  const int f = (F == kRuntimeF) ? f_bits : F;
  ...
  constexpr int A_TILE =
      BM * BK;  // 1 byte/elem -> 1 word/elem (no nibble pack)
  ...
  __shared__ uint32_t
      As_dec[BM * BKP];  // decode-on-load: one word per FP8 elem
  __shared__ uint32_t Bs_dec[BN * BKP];
...
#pragma unroll
        for (int grp = 0; grp < GROUPS_PER_TILE; grp++) {
          const DecodedFrag a_frag{&As_dec[(thread_m + tm) * BKP + grp * GS]};
          const DecodedFrag b_frag{&Bs_dec[(thread_n + tn) * BKP + grp * GS]};
          GroupResult gr =
              fp8_gdfs_group_accumulate_predecoded<GS>(a_frag, b_frag, g);
          if (gr.all_zero) {
            tile_groups[grp] = make_zero_operand();
          } else {
            tile_groups[grp] =
                group_to_operand_fp8(gr.mantissa_sum, gr.max_exp, f, g);
          }
        }
...
// The F and G lists live in mma_emu_instantiation.cuh. Each dispatch tries them
// and falls through to the kRuntime instantiation, so an unlisted value runs on
// the argument path rather than being refused.

#define VLLM_MMA_EMU_LAUNCH_FP8_COFDA(F_VAL, CHUNK)                          \
  mma_emu_scaled_fp8_mm_cofda_kernel<F_VAL, CHUNK, OutDtype>                 \
      <<<grid, block, 0, stream>>>(a, b, c, scale_a, scale_b, bias, M, N, K, \
                                   f_bits)

#define VLLM_MMA_EMU_FP8_COFDA_F_CASE(F_VAL, CHUNK) \
  if (f_bits == F_VAL) {                            \
    VLLM_MMA_EMU_LAUNCH_FP8_COFDA(F_VAL, CHUNK);    \
    return;                                         \
  }

#define VLLM_MMA_EMU_FP8_COFDA_BY_CHUNK(CHUNK)                \
  if (chunk_size == CHUNK) {                                  \
    VLLM_MMA_EMU_F_LIST(VLLM_MMA_EMU_FP8_COFDA_F_CASE, CHUNK) \
    VLLM_MMA_EMU_LAUNCH_FP8_COFDA(kRuntimeF, CHUNK);          \
    return;                                                   \
  }

template <typename OutDtype>
inline void launch_fp8_cofda(dim3 grid, dim3 block, cudaStream_t stream,
                             const __nv_fp8_storage_t* a,
                             const __nv_fp8_storage_t* b, OutDtype* c,
                             const float* scale_a, const float* scale_b,
                             const OutDtype* bias, int M, int N, int K,
                             int f_bits, int chunk_size){
    VLLM_MMA_EMU_FP8_COFDA_BY_CHUNK(16) VLLM_MMA_EMU_FP8_COFDA_BY_CHUNK(32)
    // anything else is rejected by the operator entry point
}
#undef VLLM_MMA_EMU_FP8_COFDA_BY_CHUNK
#undef VLLM_MMA_EMU_FP8_COFDA_F_CASE
#undef VLLM_MMA_EMU_LAUNCH_FP8_COFDA

#define VLLM_MMA_EMU_LAUNCH_FP8_GDFS(F_VAL, G_VAL, GROUP)                    \
  mma_emu_scaled_fp8_mm_gdfs_kernel<F_VAL, G_VAL, GROUP, OutDtype>           \
      <<<grid, block, 0, stream>>>(a, b, c, scale_a, scale_b, bias, M, N, K, \
                                   f_bits, g_bits)

#define VLLM_MMA_EMU_FP8_GDFS_G_CASE(G_VAL, F_VAL, GROUP) \
  if (g_bits == G_VAL) {                                  \
    VLLM_MMA_EMU_LAUNCH_FP8_GDFS(F_VAL, G_VAL, GROUP);    \
    return;                                               \
  }

#define VLLM_MMA_EMU_FP8_GDFS_F_CASE(F_VAL, GROUP)                      \
  if (f_bits == F_VAL) {                                                \
    VLLM_MMA_EMU_FP8_G_LIST(VLLM_MMA_EMU_FP8_GDFS_G_CASE, F_VAL, GROUP) \
    VLLM_MMA_EMU_LAUNCH_FP8_GDFS(F_VAL, kRuntimeG, GROUP);              \
    return;                                                             \
  }

#define VLLM_MMA_EMU_FP8_GDFS_BY_GROUP(GROUP)                  \
  if (group_size == GROUP) {                                   \
    VLLM_MMA_EMU_F_LIST(VLLM_MMA_EMU_FP8_GDFS_F_CASE, GROUP)   \
    VLLM_MMA_EMU_LAUNCH_FP8_GDFS(kRuntimeF, kRuntimeG, GROUP); \
    return;                                                    \
  }

template <typename OutDtype>
inline void launch_fp8_gdfs(dim3 grid, dim3 block, cudaStream_t stream,
                            const __nv_fp8_storage_t* a,
                            const __nv_fp8_storage_t* b, OutDtype* c,
                            const float* scale_a, const float* scale_b,
                            const OutDtype* bias, int M, int N, int K,
                            int f_bits, int g_bits, int group_size) {
  VLLM_MMA_EMU_FP8_GDFS_BY_GROUP(8)
  VLLM_MMA_EMU_FP8_GDFS_BY_GROUP(16)
  // anything else is rejected by the operator entry point
}

#undef VLLM_MMA_EMU_FP8_GDFS_BY_GROUP
#undef VLLM_MMA_EMU_FP8_GDFS_F_CASE
#undef VLLM_MMA_EMU_FP8_GDFS_G_CASE
#undef VLLM_MMA_EMU_LAUNCH_FP8_GDFS

template <typename OutDtype>
inline void dispatch_fp8(dim3 grid, dim3 block, cudaStream_t stream,
                         const void* a, const void* b, void* out,
                         const float* scale_a, const float* scale_b,
                         const void* bias, int M, int N, int K, int algorithm,
                         int f_bits, int g_bits, int group_size,
                         int chunk_size) {
  ...
  if (algorithm == design_space::kGDFS) {
    ...
    launch_fp8_cofda<OutDtype>(grid, block, stream, a_ptr, b_ptr, c_ptr,
                               scale_a, scale_b, bias_ptr, M, N, K, f_bits,
                               chunk_size);
  }
}
...
inline void mma_emu_scaled_fp8_mm_emu(
    ...
    design_space::MmaEmuOutDtype out_dtype, int algorithm, int f_bits,
    int g_bits, int group_size, int chunk_size, cudaStream_t stream) {
  ...
  if (out_dtype == design_space::MmaEmuOutDtype::kBFloat16) {
    detail::dispatch_fp8<__nv_bfloat16>(
        grid, block, stream, a, b, out, a_scales, b_scales, bias, M, N, K,
        algorithm, f_bits, g_bits, group_size, chunk_size);
  } else {
