...
//
// FP8 is an enumerated set rather than a range because G selects a template
// instantiation there, and the count is the product of the enumerated sets:
// opening the 25 values between 6 and 32 would multiply the build by six for
// widths nobody has asked to emulate. FP4's range is contiguous and only four
// values wide, so it stays a range.
inline constexpr int64_t G_MIN = 3;
inline constexpr int64_t FP8_G[] = {3, 4, 5, 6, 32};  // FP8 E4M3 products
