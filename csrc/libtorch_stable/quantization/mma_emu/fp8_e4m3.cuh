...
struct DecodedOperand {
  int8_t sign;  // +1 / -1
  uint8_t
      significand;  // normal: 8+mantissa (8..15); subnormal: mantissa (1..7)
...
// NaN/Inf products (significand 0, is_zero false) contribute as in the
// original. Bit-exact to fp8_multiply_unnormalized<G> + group_accumulate<G,GS>.
template <int GS>
 ...
 */
[[nodiscard]] __device__ __forceinline__ FP8Components
decompose_fp8_e4m3(uint8_t bits) {
 ...
 */
[[nodiscard]] __device__ __forceinline__ Product
fp8_multiply_unnormalized(FP8Components a, FP8Components b, int f) {
  Product result;
  result.is_nan = false;
  result.is_inf = false;
 ...
 * @return Product structure ready for FDA accumulation
 */
[[nodiscard]] __device__ __forceinline__ Product fp8_multiply(uint8_t a_bits,
                                                              uint8_t b_bits,
                                                              int f) {
 ...
 */
[[nodiscard]] __device__ __forceinline__ Product
fp8_multiply_unnormalized_g(FP8Components a, FP8Components b, int g) {
  Product result;
  result.is_inf = false;
  result.is_subnormal = false;
 ...
 * @return Product structure ready for group accumulation
 */
[[nodiscard]] __device__ __forceinline__ Product fp8_multiply_g(uint8_t a_bits,
                                                                uint8_t b_bits,
                                                                int g) {
  FP8Components a_comp = decompose_fp8_e4m3(a_bits);
  FP8Components b_comp = decompose_fp8_e4m3(b_bits);
 ...
 */
[[nodiscard]] __device__ __forceinline__ Operand
group_to_operand_fp8(int64_t mantissa_sum, int max_exp, int f, int g) {
  Operand result;
  result.is_nan = false;
  result.is_inf = false;
...
[[nodiscard]] __device__ __forceinline__ float fp8_cofda_mma(DecodedFrag a_frag,
                                                             DecodedFrag b_frag,
                                                             float c, int f) {
