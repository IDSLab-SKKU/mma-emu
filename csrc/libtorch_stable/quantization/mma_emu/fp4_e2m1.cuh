...
constexpr int MAX_PRODUCT_EXP_DIFF =
    4;                         ///< (max_exp - min_exp) for E2M1 products
 ...
 */
[[nodiscard]] __device__ __forceinline__ uint8_t load_fp4_from_tile(
    const uint8_t* tile, int local_m, int k_local, int bk_packed) {
 ...
 */
[[nodiscard]] __device__ __forceinline__ FP4Components
decompose_fp4_e2m1(uint8_t val) {
...
struct DecodedFP4Operand {
  int8_t sign;  // +1 / -1
  uint8_t
      sig;     // normal: IMPLICIT_ONE+mantissa (2/3); subnormal: mantissa (0/1)
 ...
 */
[[nodiscard]] __device__ __forceinline__ Product
fp4_multiply_unnormalized(FP4Components a, FP4Components b, int g) {
  Product result;
