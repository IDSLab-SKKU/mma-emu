 ...
 */
[[nodiscard]] __device__ __forceinline__ bool fp32_is_zero(float val) {
 ...
 */
[[nodiscard]] __device__ __forceinline__ bool fp32_is_inf(float val) {
 ...
 */
[[nodiscard]] __device__ __forceinline__ bool fp32_is_nan(float val) {
  unsigned int abs_bits = fp32_to_bits(val) & fp32::ABS_MASK;
  return abs_bits > fp32::POS_INF_BITS;
}
...
[[nodiscard]] __device__ __forceinline__ float fixed_to_fp32(
    int64_t mantissa_sum, int max_exp, int f) {
  if (mantissa_sum == 0) {
    return 0.0f;
  }
  ...
  const int final_trunc_bits =
      static_cast<int>(fp32::MANTISSA_BITS) - effective_trunc_point;
